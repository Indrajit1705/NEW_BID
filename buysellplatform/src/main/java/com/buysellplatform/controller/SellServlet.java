    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (title != null && description != null && minBidPriceStr != null && auctionEndDate != null) {
                ...
                // Parse the auction end date
                DateTimeFormatter inputFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
                DateTimeFormatter outputFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
                LocalDateTime auctionEndDateTime;

                try {
                    // Try parsing with the date-only formatter
                    auctionEndDateTime = LocalDateTime.parse(auctionEndDate, inputFormatter).withHour(0).withMinute(0).withSecond(0);
                } catch (Exception e) {
                    // Handle the exception and provide a default value
                    System.out.println("Date parsing failed: " + e.getMessage());
                    auctionEndDateTime = LocalDateTime.now().withHour(0).withMinute(0).withSecond(0);
                }
                ...
                product.setAuctionEndDate(formattedEndDate);
